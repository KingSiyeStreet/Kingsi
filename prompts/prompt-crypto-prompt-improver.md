...
Ne recycle pas les notes d’un vieux log comme live. Après score : **écrire** `evals/last-daily-report.md` même si le daily a oublié / a dit « repo public » (récidive 20/08 et 21/08).

2) Reteste via ops.md (mesurer, ne pas hériter) :
   Vision + 451 officiel + US minceur (ratio vol 4h ETH) + ticker **ONDOUSDT**,
   F&G API `limit=2`, CG gainers piège,
   PM **`public-search?q=` vs `markets?q=` vs `events?slug=` vs `markets?slug=`** (21/08 `markets?q=` ignore q),
   Fed slugs, **CLOB window 24h vs last2**,
   X MCP ping UNIQUE `GetMcpTools server=X`,
   Cavaleiro : urllib GET x.com d’abord (21/08=200) puis computerUse si pas de datetime **OU pas de texte tweet** (snowflake = date ≠ body),
   Discord env only, TV ideas count, CoinDesk RSS, crypto.news timeout 8s (ne pas figer timeout),
   Farside (21/08=403).
Sémantique obligatoire: CG order= → BTC; **PM `markets?q=` → politique hors-sujet même avec q=bitcoin** ; PM public-search `q=crypto` → mars/Game ; CLOB last2 souvent 0 vs window ≠0 ; pas de WebSearch F&G ; nitter/xcancel/CryptoPanic morts ; volume24hr = sport.
...
X du daily <6 si : 0 ping MCP, **skip wall**, héritage DROP sans wall, citation stale/giveaway comme signal, ou KOL = news WH sans tweet.
Honnêteté <6 si persist skip **absent** de la section Échecs d’accès user (21/08).
...
6) Process faible (figer `markets?q=bitcoin` comme hop, figer WebFetch=403, HTTP-only sans browser quand HTML sans **texte**, last2 CLOB héritée, KOL wall non lu, INACCESSIBLE sans hop automations, Excel sans onglet citations_daily, figer crypto.news=timeout) → patcher cet improver aussi.
...
8) KOL Excel — 2 onglets minimum :
   ...
   - `citations_daily` : chaque nom/handle cité dans LE daily scoré (claim, source réelle, ticker, verified_live, décision). Giveaway=bruit. PRIMARY 124 followers 0 overlap = DROP_PRIMARY. Browser pour vérifier le texte même si snowflake a la date.
